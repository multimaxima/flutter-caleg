...
import 'package:caleg/model/partai.dart';
import 'package:caleg/page/login.dart';
      ...
      child: Form(
        ...
        child: ListView(
          ...
          children: [
            ...
            if (idAkses == 4 && (idPartai == 0 || tingkat == "")) ...[
              Card(
                elevation: 5,
                child: Padding(
                    ...
                    children: [
                      Container(
                        ...
                        child: const Padding(
                          ...
                          child: Text(
                            "PERHATIAN !\nPengaturan pada area ini hanya bisa dilakukan 1 kali, selanjutnya Anda tidak bisa dilakukan editing",
                            style: TextStyle(
                              color: Colors.white,
                            ),
                      ...
                      DropdownSearch<PartaiModel>(
                        asyncItems: (String filter) async {
                          var response = await http.get(Uri.parse(
                              "${ApiStatus.baseUrl}/api/partai?key=$key"));

                          if (response.statusCode != 200) {
                            errorData();
                            return [];
                          } else {
                            List partai = json.decode(response.body)['data'];
                            List<PartaiModel> partaiList = [];

                            for (var element in partai) {
                              partaiList.add(PartaiModel.fromJson(element));
                            }
                        ...
                        popupProps: PopupProps.dialog(
                          itemBuilder: (context, item, isSelected) => ListTile(
                            leading: item.logo != ""
                                ? Image.memory(
                                    base64.decode(item.logo!.split(',').last),
                                    width: 40,
                                    height: 40,
                                    fit: BoxFit.cover,
                                    errorBuilder: (BuildContext context,
                            ...
                            title: Text(
                              "${item.urut}. ${item.partai.toString().toUpperCase()}",
                        ...
                        onChanged: (value) {
                          setState(() {
                            _idPartai.text = value?.id.toString() as String;
                            _partai.text =
                                value?.partai.toString().toUpperCase() ?? "";
                          });
                        },
                        selectedItem: PartaiModel(
                          id: int.tryParse(_idPartai.text),
                          partai: _partai.text.toString().toUpperCase(),
                        ),
                      ...
                      DropdownSearch<Map<String, dynamic>>(
                        ...
                        validator: (value) {
                          if (value?["tingkat"] == "") {
                            return 'Wilayah pemilihan wajib diisi';
                          }
                          return null;
                        },
                      ),
                      if (_tingkat.text == 'PROPINSI' ||
                        ...
                        DropdownSearch<PropModel>(
                          ...
                          dropdownDecoratorProps: const DropDownDecoratorProps(
                            dropdownSearchDecoration: InputDecoration(
                              labelText: "PROPINSI AREA PEMILIHAN",
                              labelStyle: TextStyle(fontSize: 14),
                              contentPadding: EdgeInsets.all(10),
                              fillColor: Colors.white,
                              filled: true,
                          ...
                          validator: (value) {
                            if (value?.noProp == "") {
                              return 'Propinsi area pemilihan wajib diisi';
                            }
                            return null;
                          },
                        ),
                      ],
                      if (_tingkat.text == 'KABUPATEN/KOTA') ...[
                        const SizedBox(height: 5),
                        DropdownSearch<KabModel>(
                          ...
                          validator: (value) {
                            if (value?.noKab == "") {
                              return 'Kabupaten/Kota area pemilihan wajib diisi';
                            }
                            return null;
                          },
                        ),
              ...
              Padding(
                ...
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("Nama Partai"),
                    Text(_partai.text.toString().toUpperCase()),
                    const SizedBox(height: 10),
                    const Text("Wilayah Pemilihan"),
                    Text(_tingkat.text.toString().toUpperCase()),
                    if (_tingkat.text == 'PROPINSI' ||
                        _tingkat.text == 'KABUPATEN/KOTA') ...[
                      const SizedBox(height: 10),
                      const Text("Propinsi"),
                      Text(_propinsiWilayah.text.toString().toUpperCase()),
                    ],
                    if (_tingkat.text == 'KABUPATEN/KOTA') ...[
                      const SizedBox(height: 10),
                      const Text("Kabupaten/Kota"),
                      Text(_kotaWilayah.text.toString().toUpperCase()),
                    ],
                  ],
                ),
              ),
                        ...
                        if (_formKey.currentState!.validate() &&
                          ...
                          var request = await http.post(
                            ...
                            body: jsonEncode(<String, String>{
                              ...
                              "id_distributor": _idDistributor.text,
                    ...
                    IconButton(
                      onPressed: () {
                        Get.dialog(
                          AlertDialog(
                            content: const Text(
                                "Anda yakin akan menghapus akun Anda ?"),
                            actions: [
                              TextButton(
                                onPressed: () async {
